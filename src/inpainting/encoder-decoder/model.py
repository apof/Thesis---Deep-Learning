...
DIR_TO_WRITE_RESULTS = "../../../../Dataset/MNIST/inpainting/encoder-decoder/predicted/"
...
def main():
  ...
  with tf.Session() as sess:
    ...
    for inp in test_inputs:
      count+=1
      imageio.imwrite(DIR_TO_WRITE_RESULTS+"input"+str(count)+".png",inp.reshape((IMAGE_HEIGHT,IMAGE_HEIGHT),order='C'))
    ...
    for i in range(test_iters):
      ...
      for i in range(len(preds[0])):
        #to preds einai lista apo listes...
        pred = preds[0][i]
        # MIN=0
        # MAX=pred.max()
        count+=1
        ...
        predicted_image = utils.fill_dark_part(test_inputs[count-1].reshape((IMAGE_HEIGHT, IMAGE_WIDTH),order='C'), pred, IMAGE_HEIGHT, IMAGE_WIDTH, WINDOW_SIZE)
